...
test('each minigame draws and routes input without browser errors', async ({ page }, info) => {
  ...
  await boot(page); await adult(page);
  /* From the registry rather than a list written out here: a game added to
     GAMES and forgotten in this array is exactly the one that would ship
     broken. */
  const kinds = await page.evaluate(() => Object.keys(GAMES));
