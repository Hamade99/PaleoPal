...
/*<data:PIX>*/
const PIX = {
  /* ---- the five action keys, the four meter glyphs and the case buttons ---- */
  'icon.feed': { w:12, h:12,
  ...
  /* ---- headgear. Drawn inside a 12x11 box, one pixel in from the edge ---- */
  'hat.frond': { w:12, h:11,
    pal:['#6f9c55'], rows:[
      '            ',
      '    00 00   ',
  ...
  /* ---- food and props. Drawn around an origin, so some rows sit above it ---- */
...
/*<data:TRI_TUNE>*/
const TRI_TUNE = {
  hipH:41,        // hip height, and so leg length
  shoulder:16,    // how far forward of centre the shoulder sits
  hipBack:26,     // how far behind centre the hip sits
  withersH:25,    // shoulder hump above the hip
  backH:20,
  bellyD:14,
  neckLen:20,     // shoulder to the back of the skull
  neckThick:15,
  headLen:22.5,   // snout length
  headDepth:10.5,
  frillW:17,      // the shield, front to back
...
/*<data:BRA_TUNE>*/
const BRA_TUNE = {
  hindH:46,       // hind limb length, and so hip height
  foreRatio:1.22, // forelimb as a multiple of it, at adult. 1.00 at hatching
  shoulder:14,
  hipBack:30,
  bodyD:44,       // deepest point, over the ribs
  neckLen:58,     // the whole neck
  neckThick:26,   // at the shoulder
  headLen:16,
  headDepth:8,
  crestH:2.05,    // nasal arch above the skull roof, at adult
  tailLen:86
};
/*</data>*/
...
function pixCanvas(id, outline){
  ...
  const c = makeCv(p.w, p.h), g = readCtx(c);
  pixDraw(g, id, p.ox || 0, p.oy || 0, 1);
  const w = p.w, h = p.h;
